  ...
  <b-card>
    ...
    >
      ...
      <template slot="items" slot-scope="row">
        ...
        <b-popover
          ...
          <div v-for="item in row.item.items">
            <h5>{{ item.name }}</h5>
            <p>Reduzido: {{ item.reducer }} Alocada: {{ item.amount }} Pendente: {{ item.amount - item.prepare }}</p>
          </div>
      ...
      <template slot="process" slot-scope="row">
        <b-progress :value="complete(row.item.items)" :max="100" show-progress animated></b-progress>
      </template>
    </b-table>
